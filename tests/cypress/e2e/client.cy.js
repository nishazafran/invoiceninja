...
function submitForm() {
    ...
    it('C-1 Shows error when all fields empty', () => {
        submitForm();
        cy.contains('success').should('be.visible'); // invalid_name translation
    });
    ...
    it('C-5 Fills valid client and contact info', () => {
        ...
        cy.get('input[id="phone_0"]').type('03001234567'); // example phone
        submitForm();
        cy.contains('success').should('be.visible'); // adjust success message
    });
