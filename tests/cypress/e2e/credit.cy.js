...
afterEach(function () {
  ...
  // Test Cases
  // --------------------
  it('P-1a Select client only and check edit exists', () => {
    selectClient('nisha');
    submitForm();
    checkEditExists();
  });

   it('P-1b Select no client', () => {
    submitForm();
     cy.contains(/save/i).should('exist');
  });

  it('P-2 Negative invoice number with invoice & due date', () => {
    selectClient('nisha');
 ...
 it('P-3 Negative invoice number with invoice & due date', () => {
  ...
  it('P-8 Fill all fields with valid random values', () => {                            n
    fillSecondDiscount('-1');                              
    submitForm();
    checkEditExists();
  });
